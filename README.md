...
A JIT implementation of [UM (Task of ICFP Programming Contest 2006)](http://www.boundvariable.org/) using Cranelift.

This can be done `sandmark.umz` in about 2.4 seconds (@ Ryzen 7950X).

```sh
$ cargo build --release && time RUST_LOG=warn ./target/release/umix-2024 run --engine=jit sandmark.umz
